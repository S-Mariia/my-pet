...
type EventBlockProps = {
  handleUpdatePet: (pet: PetData) => void;
  pet: PetData;
  type: "health" | "activity" | "nutrition";
};

function EventBlock({ handleUpdatePet, pet, type }: EventBlockProps) {
  ...
  const currentArray = pet?.[eventFieldMap[type]]?.sort(
    (a: HealthEvent, b: HealthEvent) => {
  ...
  return (
    <div className="mt-[30px] p-[15px] rounded-pet-card border border-solid border-light-text dark:border-dark-gray-bg shadow-md bg-white dark:bg-dark-gray-text">
      <p className="text-dark-gray-text dark:text-white font-600 leading-136% text-16px mb-[20px] capitalize">
        {type}
      </p>
      <div>
        {currentArray?.length ? (
          currentArray.map((i: HealthEvent) => (
            <div key={i.id}>
              {type !== "nutrition" && (
                  ...
                  <div className="flex sm:gap-[20px] sm:flex-row flex-col">
                    ...
                    <div>
                      <p className="text-dark-gray-text dark:text-white font-600 text-16px leading-150%">
                        {moment(new Date(i.date as string)).format("MMM Do YY")}
                      </p>
                    </div>
                  </div>
                  <div>
                    <button
                      ...
                      onClick={() => setIsEdit(i)}
                    >
                      <Image src={update} alt="update" height={30} width={30} />
                    </button>
                    <button
              ...
              <div>
                ...
                <p className="mb-[15px] pt-[15px] text-input-border-dark dark:text-input-border font-400 text-16px leading-150%">
                  {i.description}
                </p>
              </div>
          ...
          <p className="mb-[20px] text-input-border-dark dark:text-input-border text-center font-400 text-16px leading-150%">
            No events added
          </p>
        )}
        <div className="w-[150px] mx-[auto] mt-[20px]">
          <CustomButton
            type="button"
            title={type === "nutrition" ? "+ Add info" : "+ Add event"}
      ...
      {isDeleteEvent && (
        <DeleteEventModal
          ...
          type={type}
          events={currentArray || []}
          setIsOpenModal={setIsDeleteEvent}
          handleUpdate={handleUpdatePet}
        />
      )}
    </div>
