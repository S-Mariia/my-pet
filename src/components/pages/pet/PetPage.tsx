...
import {PetPayloadA} from "@/components/pages/add-pet/types";
import {getPet} from "@/utils/createUserInDatabase";
...
export default function PetPage({id}: PetPageProps) {
    ...
    const [isDeleteCaretaker, setIsDeleteCaretaker] = useState<Caretaker | null>(null);
    ...
    useEffect(() => {
        if (user) getPet(user, id, setPet)
    }, [user])
    const handleUpdatePet = async (payload: PetPayloadA) => {
        if (user) {
            try {const starCountRef = ref(db, 'users/' + user.uid + '/pets/' + id);
            await set(starCountRef, {
                ...pet,
                ...payload,
            });
            await getPet(user, id, setPet);
            await showSuccessToast('Pet profile has been successfully updated!')}
            catch(error) {
    ...
    return pet ? (<>
        ...
        <div className='flex mt-[30px]'>
            <div className='mb-[30px] w-[60%] pr-[30px]'>
                <div className='flex w-full justify-center items-center gap-[20px] '>
                    ...
                    <div className=' flex justify-end'><button className='w-[40px] h-[40px] hover:scale-[1.05] transition duration-300 ease-in-out'
                                                                                          onClick={() => {setIsEditPet(true)}}><Image src={update} alt="update" height={40} width={40}/></button></div>
                ...
                <div className='flex w-full justify-between items-center my-[15px]'>
                    <div><p className='text-input-border-dark dark:text-input-border font-400 text-16px leading-150%'>Size</p></div>
                    <div><p className='text-dark-gray-text dark:text-white font-600 text-16px leading-150%'>{pet.size}</p></div>
                </div>
                <div className="w-full border-input-border dark:border-input-border-dark border-[0.5px] border-solid"/>
                <div className='flex w-full justify-between items-center my-[15px]'>
                    <div><p className='text-input-border-dark dark:text-input-border font-400 text-16px leading-150%'>Weight</p></div>
                    <div><p className='text-dark-gray-text dark:text-white font-600 text-16px leading-150%'>{pet.weight}</p></div>
                </div>
                <EventBlock pet={pet} handleUpdatePet={handleUpdatePet} type={"health"}/>
                <EventBlock pet={pet} handleUpdatePet={handleUpdatePet} type={"nutrition"}/>
        ...
        {isOpenModal &&
            <AddCaretakerModal setIsOpenModal={setIsOpenModal} handleUpdate={handleUpdatePet}
                               caretakers={pet?.caretakers || []}/>}

        {!!isDeleteCaretaker &&
            <DeleteCaretakerModal isOpenModal={isDeleteCaretaker} setIsOpenModal={setIsDeleteCaretaker}
                                  handleUpdate={handleUpdatePet}
                                  caretakers={pet.caretakers || []} caretaker={isDeleteCaretaker}/>}
        {isEditPet&&<EditPetModal setIsOpenModal={setIsEditPet} handleUpdate={handleUpdatePet} pet={pet}/>}
        {isDeletePet&&<DeletePetModal isOpenModal={isDeletePet} setIsOpenModal={setIsDeletePet}  pet={pet}/>}
