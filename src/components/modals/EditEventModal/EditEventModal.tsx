...
const EditEventModal: React.FC<EditEventModalProps> = ({setIsOpenModal, handleUpdate, events, type, event}) => {
    const currentSchema = type === "nutrition" ? nutritionSchema : schema
    const currentDefaultValues = type === "nutrition" ? {description: event.description||""} : {
    ...
    const onSubmit = async (payload: HealthEvent) => {
        ...
        await handleUpdate({[type + "Events"]: updatedEventsList})
            ...
            <div onClick={(e) => {
                ...
                <div
                    ...
                    <div className={"px-8 flex flex-col  justify-between"}>
                        <h2 className=' font-700 leading-136% sm:text-28px text-20px mb-[20px] dark:text-white text-dark-gray-text'>Edit Event</h2>
                        <div
                        ...
                        <FormProvider {...methods}>
                            <form onSubmit={handleSubmit(onSubmit)}>
                                {type !== "nutrition" && <div className='min-w-[50%]'><p
                                    ...
                                    {/* @ts-ignore */}
                                    <Calendar name={'date'} errorMessage={errors?.date?.message || ''}
                                              control={control}/>
                                </div>}
                                ...
                                <div className='mt-[30px] w-[200px] mx-[auto]'><CustomButton type="submit" title="Save"
