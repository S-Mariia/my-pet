...
const DeleteCaretakerModal: React.FC<DeleteCaretakerModalProps> = ({
  isOpenModal,
  setIsOpenModal,
  handleUpdate,
  caretakers,
  ...
  const onSubmit = async () => {
    const newCaretakers = caretakers?.length
      ? caretakers.filter(
          (i) => i.name !== caretaker.name || i.email !== caretaker.email
        )
      : [];
      ...
      >
        <div className="overflow-hidden relative sm:w-[70%] w-full max-w-[600px]  py-6 rounded-2xl bg-white dark:bg-sidebar-bg">
          ...
          <div className={"px-8 flex flex-col  justify-between"}>
            ...
            <p className="text-24px my-[60px] font-600 text-center text-dark-gray-text dark:text-white leading-144%">
              Do you want to delete {caretaker.name} from your caretakers list?
            </p>

            <div className="w-[200px] mx-[auto]">
              <CustomButton type="button" title="Delete" onClick={onSubmit} />
            </div>
          </div>
        </div>
      </div>
